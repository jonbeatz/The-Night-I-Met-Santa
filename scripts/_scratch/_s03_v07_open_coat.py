#!/usr/bin/env python3
"""S3 Eyes Met v07 — add open red coat over v06 shirt/suspenders."""
...
DAY = "2026-07-22"
VERSION = "v07"
URLS_OUT = ROOT / "scripts/_scratch/_s03_v07_urls.json"

PROMPT = (
    "Edit image 1 ONLY — the S3 Eyes Met spread. KEEP everything identical: same layout, "
    "fireplace left, boy in holly PJs, Santa kneeling among gifts, tree right, gift sea, "
    "burgundy walls, warm golden light, rich oil-painting quality, eyes meeting. "
    "Image 2 = paint style. Image 3 = Santa wardrobe lock (open coat over striped shirt). "
    "ONE CHANGE ONLY: put Santa's OPEN red coat back on over his existing outfit. "
    "The coat is worn OPEN and unbuttoned — hanging open at the sides, framing his torso — "
    "NOT closed, NOT a solid red block, NOT shirtsleeves alone. "
    "White fur trim clearly visible on the coat cuffs and hem. "
    "UNDER the open coat, KEEP the cream/off-white vertically striped button-down shirt and "
    "brown leather suspenders over the shirt — both must remain clearly visible through the "
    "open coat front. Red pants and black boots unchanged. "
    "Do not change poses, faces, eye contact, room, or boy. Art only — no text."
)

NEGATIVE = (
    "shirtsleeves only, no coat, missing coat, closed coat, buttoned coat, solid red block, "
    "suspenders over coat, coat covering shirt completely, eyes not meeting, moved characters, "
    "new layout, cream walls, text, letters, watermark"
)
...
| Field | Value |
|-------|--------|
| **name** | S3 Eyes Met — open red coat over v06 shirt/suspenders |
| **unit** | S03-eyes-met |
| **book page** | Flow v2 p8\\|9 · FULL SPREAD |
| **version** | {VERSION} |
| **date** | {DAY} |
| **model** | `fal-ai/qwen-image-2/pro/edit` |
| **settings** | 2048×1024 · edit v06 + style-lock-v2 + santa-G0.png |
| **seed** | {seed} |
| **request_id** | `{req_id}` |
| **fal_url** | `{url}` |
| **status** | working — open coat + striped shirt + brown suspenders visible |
| **previous** | v06 |

## Change vs v06

Add open unbuttoned red coat (white fur cuffs/hem) framing the sides.  
Keep striped shirt + brown suspenders visible under the open coat. Layout/eyes unchanged.
""",
        encoding="utf-8",
    )
...
def build_board(im: Image.Image, out: Path) -> None:
    ...
    d.text(
        (margin, 44),
        "Open red coat over striped shirt + brown suspenders · layout from v06",
        fill=(110, 100, 90),
        font=font(13),
    )
    y = margin + header
...
def upload_refs() -> dict:
    load_env()
    urls = {
        "v06": upload(V06, "s03-v06-base.png", (2048, 1024)),
            ...
            json.dumps(
                {
                    ...
                    "url": url,
                    "model": "fal-ai/qwen-image-2/pro/edit",
                    "refs": ["v06", "style-lock-v2", "santa-G0.png"],
                    "fix": "open red coat over v06 shirt/suspenders",
                    "previous": "v06",
                },
                indent=2,
            ),
