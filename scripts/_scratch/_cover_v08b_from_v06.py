#!/usr/bin/env python3
"""Cover v08b — skip broken v07. Original pose + v06 paint: single boy lean, Santa toward tree, no poster.
...
PROMPT = (
    "Edit into one children's book COVER — ART ONLY, square. Do NOT invent a second head. "
    "Image 2 = v06 BEST BASE — KEEP deep rich saturated burgundy walls, warm glowing Christmas tree, "
    "fireplace, armchair, presents, Santa in red hat kneeling at the sack, thick painting texture. "
    "REMOVE the framed poster/photo from the dark hallway — empty dark wall only. "
    "Image 1 = ORIGINAL COVER — the ONLY boy pose reference: ONE boy only, back/side view, "
    "leaning FORWARD into the lit doorway so his head is clearly inside the frame (more to the right), "
    "hand on the doorframe — copy this lean exactly. Also copy Image 1 Santa HEAD: turned toward "
    "the Christmas tree / away from camera so his FACE is mostly hidden (hat + back of head + beard "
    "edge only — NOT a clear left-facing facial profile looking across the room). "
    "Image 3 = Boy G0 face/hair/PJs identity on Image 1's single peek pose. "
    "Merge: Image 2 room + deep paint + no hallway poster; Image 1 single-boy lean + Santa head "
    "toward tree (face hidden); Image 3 boy identity. Exactly one child. Scrub title lettering."
)
NEGATIVE = (
    "two heads, double head, duplicate head, twin boy, ghost head, second face, double exposure, "
    "extra child, hallway poster, framed picture beside boy, photo on hallway wall, "
    "Santa clear facial profile looking left, Santa face fully visible, Santa looking at boy, "
...
def write_docs(seed: int, urls: list[str], result_url: str) -> None:
    (OUT / "RECIPE.md").write_text(
        f"""# RECIPE — Cover / v08b-single-boy-from-v06

| Field | Value |
|-------|--------|
| **version** | v08b-single-boy-from-v06 |
| **date** | {DAY} |
...
| **note** | Skips v07/v08 (double-head poison) |

## Goals

- One boy leaning into doorway (original pose)
- Santa head toward tree, face mostly hidden
- No hallway poster
...
def build_board(alt: Image.Image) -> Path:
    ...
    draw.text(
        (pad, 12),
        "Cover — beige-v2 KEEP  vs  v08b (from v06+original · no v07 double-head)",
        fill=(40, 40, 40),
        font=font,
    )
    ...
    draw.text((pad * 2 + GEN, h - 26), "v08b dial", fill=(80, 80, 80), font=font_sm)
    INDEX.mkdir(parents=True, exist_ok=True)
    out = INDEX / "Cover-beige-v2-vs-v08b-board.png"
...
def main() -> None:
    ...
    print("upload original + v06 + boy G0 (skip v07)")
    urls = [
    ...
    im.crop((0, 400, 700, 1600)).resize((350, 600)).save(
        INDEX / "cover-v08b-boy-crop.png", optimize=True
    )
    print("board", board)
    print("DONE v08b seed", seed)
