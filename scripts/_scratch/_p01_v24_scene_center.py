"""P01 v24: scenery CENTER band — open text wash TOP + BOTTOM. FRAME ON."""
...
REFS = [
    ...
    ROOT / "Media" / "generated" / "mocks" / "P01-title" / "v23" / "art-P01-title-gemini-fal.png",
    ROOT / "Images" / "styles2" / "spread-Frame-Style1.png",
]

PROMPT = """Create an alternate of this title-page illustration — SAME paint style, watercolor frame,
fireplace+stockings LEFT, Christmas tree RIGHT, simplified decor, soft heirloom look.

CRITICAL COMPOSITION — scenery in the MIDDLE:
- Place the scenic vignette as a horizontal band in the CENTER of the square.
- Leave generous open cream/soft watercolor wash for typography at BOTH the TOP and the BOTTOM
  (roughly balanced — enough calm paper above AND below for title/author later).
- Scale the scenery a bit SMALLER if needed so those top and bottom text bands feel comfortable
  (not cramped). Soft feathered painted edges into the open paper.

KEEP:
- WATERCOLOR FRAME ON: soft irregular white/cream vignette (styles2 look)
...
| **model** | `{ENDPOINT}` |
| **FRAME** | **ON** |
| **layout** | scenery **CENTER** · open text wash **TOP + BOTTOM** |
