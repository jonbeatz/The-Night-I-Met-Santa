#!/usr/bin/env python3
"""Cover v11 — conservative Banana: KEEP original boy+Santa poses byte-stable; only paint/scrub.
...
OUT = DEV / "v11-conservative-paint-scrub"
...
PROMPT = (
    "Surgical edit of Image 1 only. CRITICAL: do NOT change the boy's body or head pose at all — "
    "keep exactly ONE head, same lean into the doorway as Image 1 (no second head, no double face). "
    "CRITICAL: do NOT change Santa's body or head orientation at all — keep his head turned away "
    "toward the tree with face mostly hidden exactly like Image 1 (do not rotate his head to show "
    "a left profile face). "
    "ALLOWED changes only: "
    "(1) Remove all title text / gold lettering / Written By Jack Farrell — paint over with "
    "matching ceiling/wall. "
    "(2) Remove the framed picture on the dark hallway wall — plain wall. "
    "(3) Deepen and saturate colors to match Image 2's rich burgundy walls and warm painterly "
    "tree glow — thicker paint texture, richer shadows — WITHOUT changing character poses. "
    "ART ONLY. Square. No typography."
)
...
def write_docs(seed, urls: list[str], result_url: str) -> None:
    (OUT / "RECIPE.md").write_text(
        f"""# RECIPE — Cover / v11-conservative-paint-scrub

| Field | Value |
|-------|--------|
| **version** | v11-conservative-paint-scrub |
| **date** | {DAY} |
| **model** | `fal-ai/nano-banana-pro/edit` @ 2K |
| **seed** | {seed} |
| **print** | **{PRINT}×{PRINT}** @ 300 DPI |
| **status** | dial |
| **strategy** | Freeze Image1 boy+Santa poses; only scrub text/poster + deepen paint toward v06 |

## Goals

- Preserve original single-boy lean + Santa face-hidden
- Remove hallway poster + title text
- Deeper paint like v06
""",
        encoding="utf-8",
    )
...
def build_board(alt: Image.Image) -> Path:
    ...
    draw.text(
        (pad, 12),
        "Cover — beige-v2 KEEP  vs  v11 (freeze poses · scrub text/poster · deepen paint)",
        fill=(40, 40, 40),
        font=font,
    )
    ...
    draw.text((pad * 2 + GEN, h - 26), "v11 dial", fill=(80, 80, 80), font=font_sm)
    INDEX.mkdir(parents=True, exist_ok=True)
    out = INDEX / "Cover-beige-v2-vs-v11-board.png"
...
def main() -> None:
    ...
    im.crop((0, 400, 700, 1600)).resize((350, 600)).save(
        INDEX / "cover-v11-boy-crop.png", optimize=True
    )
    im.crop((900, 700, 1800, 1800)).resize((450, 450)).save(
        INDEX / "cover-v11-santa-crop.png", optimize=True
    )
    print("board", board)
    print("DONE v11 seed", seed)
