...
src = ROOT / "Media/development/S12-god-bless/v18b"
...
note = (
    "S12-god-bless v18b LOOK preferred (closest) · v19/v19b failed 9-deer restore "
    "(collapsed to ~5 then ~3) · still need 4 pairs+Rudolph"
)
for p in root["plates"]:
    ...
    if p["id"] in ("p28", "p29"):
        p.update({"version": "merged-v18b", "status": "merged", "date": "2026-07-23", "notes": "Absorbed — " + note})
for d in root["verdicts"]:
    if d.get("page") == "26|27":
        d.update({"version": "v18b", "status": "working", "date": "2026-07-23", "notes": note})
    if d.get("page") == "28|29":
        d.update({"version": "merged-v18b", "status": "merged", "date": "2026-07-23", "notes": "MERGED — " + note})
